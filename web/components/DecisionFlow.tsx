...
const NODES: { icon: ReactNode; title: string; sub: string; accent: string }[] = [
  ...
  { icon: <Gauge className="h-4 w-4" />, title: "Detector", sub: "calibrated LightGBM + SHAP + anomaly", accent: "text-indigo-500" },
  { icon: <SlidersHorizontal className="h-4 w-4" />, title: "Decision core", sub: "deterministic rules → cost-optimal band", accent: "text-violet-500" },
  { icon: <Bot className="h-4 w-4" />, title: "Agent (amber)", sub: "typed tools → policy RAG → structured decision", accent: "text-fuchsia-500" },
          ...
          <div className="flex flex-col items-center">
            ...
            {i < NODES.length - 1 && (
              <div className="relative my-1 h-7 w-px bg-line">
                <span
                  className="flow-dot-v absolute left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-indigo-500"
